...
async function getLikesByUserId(ctx: RouterContext, next: any) {
  const userId = parseInt(ctx.params.userId);
  const result = await likes.getLikesByUserId(userId); // Implement this method in the likes model
  ctx.body = result;
  await next();
}
async function getFavsByUserId(ctx: RouterContext, next: any) {
  const userId = parseInt(ctx.params.userId);
  const result = await favs.getFavsByUserId(userId); // Implement this method in the likes model
  ctx.body = result;
  await next();
}
...
router.get("/likes/user/:userId", basicAuth, getLikesByUserId);
router.get("/fav/user/:userId", basicAuth, getFavsByUserId);
