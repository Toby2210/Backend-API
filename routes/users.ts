...
const forgotPassword = async (ctx: any, next: any) => {
  const { username, email } = ctx.request.body;
  console.log(ctx.request.body)
  let id = await model.compare(username, email);
  console.log(id)
  if (id!==null) {
    ctx.status = 200;
    ctx.body = {id};
    ...
    ctx.status = 404;
    ctx.body = "No matching record found for the provided username and email.";
  }
  await next();
};
...
router.post("/forgot", bodyParser(),forgotPassword);
