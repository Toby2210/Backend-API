...
export const getLikesByUserId = async (uid: any) => {
  let query = `SELECT articleid FROM articleslikes WHERE userid = ${uid}`;
  const result: any = await db.run_query(query, [uid]);
  return result.map((row: any) => row.articleid);
};
